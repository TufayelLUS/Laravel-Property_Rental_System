...
<div class="container my-4">
	<h1 class="mb-5">Edit Property Information</h1>
	<form class="text-left font-weight-normal border" style="padding: 20px;" method="POST" enctype="multipart/form-data">
		...
		<input class="form-control mb-3 mt-1" type="text" name="title" placeholder="Property Name* (ex. Shaheen Villa for rent)" value="{{ $listing->title }}" minlength="10" required>
		...
		<select name="cat" id="cat" class="form-control mb-3 mt-1" required>
		...
		<input min="0" type="number" class="form-control mb-3 mt-1" name="property_price" min="0" value="{{ $listing->property_price }}" placeholder="Property Rent/Month*" required>
		...
		<span id="rooms_label">
		Rooms</span>
		<span id="rooms_tooltip_normal"><i data-toggle="tooltip" data-placement="top" title="Number of rooms available" class="fas fa-info-circle text-info"></i></span>
		...
		<input min="0" type="number" class="form-control mb-3 mt-1" placeholder="Rooms" name="rooms" value="{{ $listing->rooms }}" required>
		...
		<input min="0" type="number" class="form-control mb-3 mt-1" placeholder="Bedrooms" name="bedrooms" value="{{ $listing->bedrooms }}">
		...
		<input min="0" type="number" class="form-control mb-3 mt-1" placeholder="Bathrooms" name="bathrooms" value="{{ $listing->bathrooms }}">
		...
		<input min="0" type="number" class="form-control mb-3 mt-1" placeholder="Attached Bathrooms" name="attached_bathrooms" value="{{ $listing->attached_bathrooms }}">
		...
		<input min="0" type="number" class="form-control mb-3 mt-1" placeholder="Common Bathrooms" name="common_bathrooms" value="{{ $listing->common_bathrooms }}">
		</span>
		...
		<input min="0" type="number" class="form-control mb-3 mt-1" placeholder="Kitchens" name="kitchens" value="{{ $listing->kitchens }}">
		...
		<input min="0" type="number" class="form-control mb-3 mt-1" placeholder="Balcony" name="balcony" value="{{ $listing->balcony }}">
		</span>
		...
		<div class="text-left">
			<span class="form-inline" id="gas_block">
		...
		<div class="text-left">
			...
			<input type="checkbox" class="" checked disabled> Via Email <span class="font-weight-normal">(we will use this by default)</span><br>
		</div>
		...
		<input class="form-control mb-3" type="file" name="photo[]" multiple>
		...
		<textarea name="property_location" class="form-control mb-3" placeholder="Property Location* (e.g. House 46/C, Road No. 2, Rajpara R/A, Tilagor, Sylhet)" rows="3" required>{{ $listing->property_location }}</textarea>
		...
		<textarea minlength="20" class="form-control mb-3" placeholder="Property Details*" rows="10" name="desc" required>{{ $listing->description }}</textarea>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$('option[value="{{ $listing->cat }}"]').prop("selected", "selected");
		...
		@elseif($listing->expiry_unit == -1)
		$('option[value="6"]').prop("selected", "selected");
		@endif
		$('option[value="{{ $listing->applicant_type }}"]').prop("selected", "selected");
		@if($listing->gas == "Regular" || $listing->gas == "Cylinder")
		$('input[name="gas"]').prop('checked', 'true');
		$('option[value="{{ $listing->gas }}"]').prop("selected", "selected");
		$('select[name="gas_type"]').prop("disabled", false);
		@endif
		@if ($listing->electricity == "Prepaid" || $listing->gas == "Postpaid")
		$('input[name="electricity"]').prop('checked', 'true');
		$('option[value="{{ $listing->electricity }}"]').prop("selected", "selected");
		$('select[name="electricity_type"]').prop("disabled", false);
		@endif
		@if ($listing->phone_shared == "on")
		$('input[name="phone_shared"]').prop('checked', 'true');
		@endif
		@if ($listing->advance != "Not Required")
		$('input[name="advance"]').prop('checked', 'true');
		$('input[name="advance_amount"]').val(@php
			echo intval($listing->advance)
			@endphp);
		$('input[name="advance_amount"]').prop("disabled", false);
		@endif
		@if ($listing->furniture == "Included")
		$('input[name="furniture"]').prop('checked', 'true');
		@endif
		@if ($listing->swimming == "Included")
		$('input[name="swimming"]').prop('checked', 'true');
		@endif
		@if ($listing->parking == "Included")
		$('input[name="parking"]').prop('checked', 'true');
		@endif
		@if ($listing->playground == "Included")
		$('input[name="playground"]').prop('checked', 'true');
		@endif
